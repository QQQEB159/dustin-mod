...
var sliderArea:FlxSprite;
var zButton:FlxSprite;
var zText:FlxText;
...
function create() {
	...
	isMobile = (FlxG.onMobile || FlxG.onAndroid || FlxG.onIOS);
	
	sliderArea = new FlxSprite().makeGraphic(300, 300, 0x40FF0000);
	sliderArea.cameras = [camUndertale];
	sliderArea.alpha = 0;
	add(sliderArea);//乐

	zButton = new FlxSprite().makeGraphic(90, 90, 0xFF666666);
	zButton.cameras = [camUndertale];
	zButton.updateHitbox();
	zButton.antialiasing = false;
	zButton.alpha = 0;
	add(zButton);

	zText = new FlxText(0, 0, 0, "Z", 34);
	zText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, "center");
	zText.cameras = [camUndertale];
	zText.alpha = 0;
	add(zText);
...
public function box_open_animation() {
	...
	FlxTween.num(0, 1, 1, null, (val:Float) -> {
		...
		sliderArea.alpha = Math.floor(val*10)/10;
		zButton.alpha = Math.floor(val*10)/10;
		zText.alpha = Math.floor(val*10)/10;
	});
...
public function switch_soul_mode(mode:Int) {
	...
	sliderArea.colorTransform.color = mode;
...
function update(elapsed:Float) {
	...
	var zButtonPressed = false;

	if (isMobile) {
		for (touch in FlxG.touches.list) {
			var touchX = touch.getScreenPosition(camUndertale).x;
			var touchY = touch.getScreenPosition(camUndertale).y;
			
			if (touchX >= sliderArea.x && touchX <= sliderArea.x + sliderArea.width &&
				touchY >= sliderArea.y && touchY <= sliderArea.y + sliderArea.height) {
				
				isDragging = true;
				var relX = (touchX - sliderArea.x) / sliderArea.width;
				var relY = (touchY - sliderArea.y) / sliderArea.height;
				soulHitbox.x = battleBox.x + (relX * (battleBox.extra["bWidth"] - soulHitbox.width));
				soulHitbox.y = battleBox.y + (relY * (battleBox.extra["bHeight"] - soulHitbox.height));
				
				soulHitbox.x = Math.max(battleBox.x, Math.min(soulHitbox.x, battleBox.x + battleBox.extra["bWidth"] - soulHitbox.width));
				soulHitbox.y = Math.max(battleBox.y, Math.min(soulHitbox.y, battleBox.y + battleBox.extra["bHeight"] - soulHitbox.height));
			}
			
			if (touch.justPressed) {
				if (touchX >= zButton.x && touchX <= zButton.x + zButton.width &&
					touchY >= zButton.y && touchY <= zButton.y + zButton.height) {
					zButtonPressed = true;
					zButton.color = 0xFF333333;
				}
			}
		}
	} else {
		if (FlxG.mouse.pressed) {
			var mouseX = FlxG.mouse.getScreenPosition(camUndertale).x;
			var mouseY = FlxG.mouse.getScreenPosition(camUndertale).y;
			
			if (mouseX >= sliderArea.x && mouseX <= sliderArea.x + sliderArea.width &&
				mouseY >= sliderArea.y && mouseY <= sliderArea.y + sliderArea.height) {
				
				isDragging = true;
				var relX = (mouseX - sliderArea.x) / sliderArea.width;
				var relY = (mouseY - sliderArea.y) / sliderArea.height;
				soulHitbox.x = battleBox.x + (relX * (battleBox.extra["bWidth"] - soulHitbox.width));
				soulHitbox.y = battleBox.y + (relY * (battleBox.extra["bHeight"] - soulHitbox.height));
		...
		if (FlxG.mouse.justPressed) {
			...
			if (mouseX >= zButton.x && mouseX <= zButton.x + zButton.width &&
				mouseY >= zButton.y && mouseY <= zButton.y + zButton.height) {
				zButtonPressed = true;
				zButton.color = 0xFF333333;
			}
		}
	...
	if (zButtonPressed && soulMode == SOUL_YELLOW) {
		shoot();
	}

	if (!FlxG.mouse.pressed && FlxG.touches.list.length == 0) {
		zButton.color = 0xFF666666;
	}
...
function undertale_update(elapsed:Float) {
	...
	sliderArea.x = FlxG.width - sliderArea.width - 930;
	sliderArea.y = FlxG.height - sliderArea.height - 30;

	zButton.x = sliderArea.x - zButton.width - -1100;
	zButton.y = sliderArea.y + 80;
	zText.x = zButton.x + (zButton.width - zText.width) / 2;
	zText.y = zButton.y + (zButton.height - zText.height) / 2;
	...
	if (invisTimer > 0) {
		...
		if (soulHitbox.ID > 2) {
			...
			if (soulHitbox.ID > 4) soulHitbox.ID = 0;
		}
	} else {
		soulHitbox.ID = 0;
