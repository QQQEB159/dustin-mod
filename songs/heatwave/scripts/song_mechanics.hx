...
public function box_open_animation() {
    ...
    FlxTween.num(0, 1, 1, null, (val:Float) -> {
        ...
        joystickBg.alpha = Math.floor(val*10)/10;
        joystickStick.alpha = Math.floor(val*10)/10;
...
function update(elapsed:Float) {
    ...
    if (!justPressedZ && getZPressed()) justPressedZ = true;
...
function undertale_update(elapsed:Float) {
    ...
    if (getJoystickLeft()) soulHitbox.x -= sframeSpeed;
    if (getJoystickRight()) soulHitbox.x += sframeSpeed;
    if (getJoystickUp()) soulHitbox.y -= sframeSpeed;
    if (getJoystickDown()) soulHitbox.y += sframeSpeed;
