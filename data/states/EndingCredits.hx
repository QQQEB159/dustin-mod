...
function create() {
    ...
    script.set("onStart", () -> {
        ...
        if (menuType == "genocide") {
            ...
            createText("NexusVGM", 850, 3540);
            createText("HIGGAMEON", 833, 9990);
            createText("Ralty", 880, 9155);
            ...
            createText("HIGGAMEON", 850, 9030);
            createText("Ralty", 895, 9080);
            ...
            createText("HIGGAMEON", 905, 22611).velocity.y = -220;
            createText("Ralty", 955, 22656).velocity.y = -220;
            moveVel = -220;
...
function createText(string, x, y) {
    ...
    txt.setFormat(Paths.font("8bit-jve.ttf"), 52, 0xFFFFFFFF, 'center', FlxTextBorderStyle.NONE, FlxColor.BLACK);
    txt.camera = fakeCam;
    txt.angle = y;
    txt.onDraw = (spr) -> {
        if (updatePos)
