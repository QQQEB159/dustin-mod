...
function createWeekSprites() {
    ...
    for(key in weekList.keys()) {
        ...
        if(data.key != null) {
            ...
            spr.ID = 0;
            ID++;
        } else spr.ID = 1; // unlocked
        data.spr = spr;
        add(spr);
        weekSprites.push(spr);

        final score:Float = FunkinSave.getWeekHighscore(data.week.id, data.week.difficulties[0]).score;
        data.unlocked = score != 0;
        /* DEV NOTE (hi ralty)
         * if you want to test the unlock animations, do
         * if(score < -1)
         * but know that it will automatically unlock the weeks for you
        */
        if(score > 1) {
            spr.ID = 1;
            ...
            if(Options.gameplayShaders && FlxG.save.data.water)
                spr.shader = water;
            if(data.key != null && dustinShop["keys"][key]) {
                quedLocks.push(key);
            }
        }
    }
...
function createLockSequence() {
    ...
    if(Options.gameplayShaders) {
        if (FlxG.save.data.chromwarp) {
            ...
            FlxTween.tween(distortion, {distortion: 0.15}, 1.25, {ease: FlxEase.quartOut});
        }
        ...
        if (FlxG.save.data.water) {
            ...
            FlxTween.tween(camWater, {strength: 0.15}, 1.25, {ease: FlxEase.quartOut});
        }
    }
    var ID:Int = -1;
...
function loadLocks() {
    ...
    new FlxTimer().start(1.05, function(_) {
        ...
        if(Options.gameplayShaders && FlxG.save.data.warp)
        ...
        FlxTween.tween(key, {'scale.x': 0.1, 'scale.y': 0.1, alpha: 0}, 1.2, {ease: FlxEase.circInOut, onComplete: (_) -> {
            ...
            FlxTween.num(FlxG.save.data.antiFlash ? 0.15 : 2, 0, 2, {
                ...
                onComplete: () -> {
                    ...
                    FunkinSave.setWeekHighscore(keyData.week.id, 'hard', {
                        score: 2,
                        misses: 0,
                        accuracy: 0,
                        hits: [],
            ...
            if (Options.gameplayShaders) {
                ...
                if (FlxG.save.data.warp) FlxTween.tween(distortion, {distortion: intensity}, 1, {ease: FlxEase.sineInOut});
            }
...
function update(elapsed:Float) {
    ...
    if(canLerp) {
        ...
        if(camKey != null) {
            ...
            if(Options.gameplayShaders && FlxG.save.data.warp)
                warp.distortion = distortion.distortion * 2;
        }
    }
