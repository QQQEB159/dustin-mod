...
function update(elapsed:Float) {
    ...
    if (shaketime > 0) {
        ...
        if(FlxG.save.data.mWindow && !window.fullscreen) {
            var shake_w:Float = (Lib.current.stage.fullScreenWidth - Lib.application.window.width) * (monitorX == 0 ? .05 : 0.1);
            var shake_h:Float = (Lib.current.stage.fullScreenHeight - Lib.application.window.height) * (monitorY == 0 ? .1 : 0.2);
            DustinUtil.window.set(
                (FlxG.random.float(-shake_w, shake_w) * shaketime) + ((monitorX * 0.025) * (Lib.current.stage.fullScreenWidth - Lib.application.window.width) * .1),
                (FlxG.random.float(-shake_h, shake_h) * shaketime) + ((monitorY * 0.025) * (Lib.current.stage.fullScreenHeight - Lib.application.window.height) * .1)
            );
        }

        shaketime -= elapsed;
    } else if((FlxG.save.data.mWindow && !window.fullscreen) && (prev_monitorX != (prev_monitorX = monitorX) || prev_monitorY != (prev_monitorY = monitorY))) {
        DustinUtil.window.set(
            (monitorX * 0.025) * (Lib.current.stage.fullScreenWidth - Lib.application.window.width) * .1,
            (monitorY * 0.025) * (Lib.current.stage.fullScreenHeight - Lib.application.window.height) * .1
        );
    }
