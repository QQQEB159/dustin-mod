    ...
    if(Assets.exists(jsonPath)) {
        ...
        try {
            ...
            for(key in raw.keys) {
                ...
                var bought:Bool = FlxG.save.data.dustinBoughtStuff.contains(weekName + ' key') || FunkinSave.getWeekHighscore(key.week + "-1", "hard").score > 0;
                
                if(key.score == 0 && FlxG.save.data.dustinBoughtStuff.contains(weekName + ' key')) {
