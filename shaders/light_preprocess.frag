...
vec4 preproccess_tex(vec2 uv) {
    ...
    if (float(flixel_texture2D(bitmap, uv).rgb) > threshold)  {
        vec4 col = vec4(1.0);
        col.r *= lightcol.r/255.0;
        col.g *= lightcol.g/255.0;
        col.b *= lightcol.b/255.0;
        return col;
    }
...
vec4 crepuscular_rays(vec2 texCoords, vec2 pos) {
    ...
    float weight = (0.17 * bright) + sin(time)*0.02; 
...
void main()
{
    ...
    vec4 col = crepuscular_rays(uv, vec2(0.5+ cos(time+0.32/2.0)*0.04, 0.3 + sin(time/2.0)*0.03));
    col *= 1.0;
