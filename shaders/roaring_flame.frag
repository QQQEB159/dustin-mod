...
void main() {
	...
	float dist = 8.5-sin(time*0.4)/1.89;
    
    vec2 p = (gl_FragCoord.xy*zoom) * dist / openfl_TextureSize.xx;
    p.x -= time/1.1;
    p.y = 1.0-p.y;
    ...
    vec3 color = vec3(c * cos(shift * gl_FragCoord.y / openfl_TextureSize.y));
    color += .05;
    color.r *= .9;
    vec3 hsv = rgb2hsv(color*intensitiy);
    hsv.y *= hsv.z  * 1.1;
    hsv.z *= hsv.y * 1.13;
    hsv.y = (2.2-hsv.z*.9)*1.20;
    color = hsv2rgb(hsv);
    gl_FragColor = vec4(color.x, color.y, color.z, intensitiy);
}