...
void main()
{
    ...
    float pixMultiplier = pixely ? 1.6 : 1.0;
    float timeSpeed = SPEED * time;
    float time4 = time * 0.4;
    int maxLayers = min(LAYERS, 10);

    for (int i = STARTING_LAYERS; i < maxLayers; i++) {
        float fi = float(i);
        float layerDiff = float(maxLayers - i);
        float depthScale = 1.0 + fi * DEPTH;
        vec2 q = uvCentered * depthScale;
        q += vec2(
            q.y * (widthScale * fract(fi * 7.238917) - widthScale * 0.5) + (SPEED * layerDiff * 0.2) * time4,
            timeSpeed / (1.0 + fi * DEPTH * 0.03)
        );
    ...
    vec4 rect = vec4(
        (snowMeltRect.x / openfl_TextureSize.x) * res.x,
    ...
    vec3 effect = vec3(acc) * 0.5;
    flixelColor.rgb += effect * pixMultiplier * BRIGHT;
