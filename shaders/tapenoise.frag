...
float nn(vec2 p){
    ...
    float v = (n(vec3(y*0.01 +s, 			1.0, 1.0)) + 0.0)
...
void main(){
    ...
    // Reduce block size for better performance
    float linesN = 80.0; //fields per seconds (reduced from 120)
    float one_y = res.y / linesN; //field line
    uv = floor(uv * 2.0 * res.xy / one_y) * one_y;
    ...
    // Apply noise with strength control
    vec3 finalColor = baseColor + vec3(noise * 0.2 * strength);
